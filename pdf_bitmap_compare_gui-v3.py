...
class PDFBitmapCompareApp:
    ...
    def display_image(self, canvas, tk_img):
        canvas.delete("all")
        # Получаем размеры canvas и картинки
        canv_w = int(canvas.winfo_width())
        canv_h = int(canvas.winfo_height())
        img_w, img_h = tk_img.width(), tk_img.height()
        # Центр по координатам canvas
        x = (canv_w - img_w) // 2 if canv_w > img_w else 0
        y = (canv_h - img_h) // 2 if canv_h > img_h else 0
        canvas.create_image(x, y, anchor="nw", image=tk_img)
        canvas.image = tk_img
        canvas.config(scrollregion=(0, 0, max(canv_w, img_w), max(canv_h, img_h)))
        if self.diff_boxes:
    ...
    def draw_diff_boxes(self):
        for idx, (x, y, w, h) in enumerate(self.diff_boxes):
            ...
            for canvas in [self.canvas_original, self.canvas_final]:
                # Центрирование прямоугольников
                canv_w = int(canvas.winfo_width())
                canv_h = int(canvas.winfo_height())
                img = canvas.image
                img_w = img.width()
                img_h = img.height()
                dx = (canv_w - img_w) // 2 if canv_w > img_w else 0
                dy = (canv_h - img_h) // 2 if canv_h > img_h else 0
                canvas.create_rectangle(
                    x0 + dx, y0 + dy, x1 + dx, y1 + dy,
                    outline=color, width=2, tags=f"diffbox_{idx}"
                )
    ...
    def on_canvas_click(self, event):
        if self._hand_mode:
            return
        canvas = event.widget
        # Получаем смещение картинки относительно Canvas
        canv_w = int(canvas.winfo_width())
        canv_h = int(canvas.winfo_height())
        img = canvas.image
        img_w = img.width()
        ...
        dx = (canv_w - img_w) // 2 if canv_w > img_w else 0
        dy = (canv_h - img_h) // 2 if canv_h > img_h else 0
        x = (canvas.canvasx(event.x) - dx) / self.scale_factor
        y = (canvas.canvasy(event.y) - dy) / self.scale_factor
    ...
    def center_on_box(self, x, y, w, h):
        for canvas in [self.canvas_original, self.canvas_final]:
            ...
            dx = (width - img_width) // 2 if width > img_width else 0
            dy = (height - img_height) // 2 if height > img_height else 0
            sx = max(0, ((x + w / 2) * self.scale_factor + dx - width // 2) / (img_width - width + 1))
            sy = max(0, ((y + h / 2) * self.scale_factor + dy - height // 2) / (img_height - height + 1))
    ...
    def update_zoom(self, val):
        ...
        for canvas in [self.canvas_original, self.canvas_final]:
            ...
            img_w = img.width()
            img_h = img.height()
            dx = (width - img_w) // 2 if width > img_w else 0
            dy = (height - img_h) // 2 if height > img_h else 0
            cx = (canvas.canvasx(width // 2) - dx) / prev_scale
            cy = (canvas.canvasy(height // 2) - dy) / prev_scale
        ...
        for idx, canvas in enumerate([self.canvas_original, self.canvas_final]):
            ...
            dy = (height - img_h) // 2 if height > img_h else 0
            cx, cy = centers[idx]
            x0 = max(0, (cx * self.scale_factor + dx - width // 2) / (img_w - width + 1))
            y0 = max(0, (cy * self.scale_factor + dy - height // 2) / (img_h - height + 1))
...
if __name__ == "__main__":
    ...
    # Максимизация окна на всю рабочую область
